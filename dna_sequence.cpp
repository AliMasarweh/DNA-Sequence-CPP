...
DNASequence::DNASequence(const char *dnaSeq): m_sequence(dnaSeq) {
...
DNASequence::DNASequence(const string &dnaSeq):m_sequence(dnaSeq) {
    if(!this->validSequence()){
        throw InvalidNucleotideDNASequence();
    }
}
...
DNASequence::DNASequence(const DNASequence &dnaSequence)
        :m_sequence(dnaSequence.m_sequence) {}
...
void DNASequence::readFromFile(string path) {
    ifstream file;
    file.open(path.c_str());
    string tmp;
    getline(file, tmp);
